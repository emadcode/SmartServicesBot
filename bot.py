...
# رابط استضافتك الأساسي (مثلاً: https://your-app.onrender.com)
...
# ==========================================
# 3. توليد روابط التفعيل الفعالة (بدون 404)
# ==========================================
...
# صفحة ويب احترافية تظهر للعميل عند فتح رابط التفعيل دون أي أخطاء 404
...
# ==========================================
# 5. لوحة تحكم الأدمن (مع زر توليد روابط التفعيل الفعالة مجاناً)
# ==========================================
...
def open_admin_panel(chat_id):
    markup = InlineKeyboardMarkup(row_width=1).add(
        InlineKeyboardButton("🔗 توليد رابط تفعيل فعال (مجاناً بدون رصيد)", callback_data="adm_gen_active_link"),
...
def admin_callbacks(call):
    ...
    action = call.data
    if action == 'adm_gen_active_link':
        bot.answer_callback_query(call.id)
        msg = bot.send_message(call.message.chat.id, "🔗 **أرسل الآن اسم الخدمة أو نوع الاشتراك لتوليد رابط التفعيل الفعال:**", parse_mode="Markdown")
        bot.register_next_step_handler(msg, process_admin_active_link_generation)
...
def process_admin_active_link_generation(message):
    if str(message.chat.id) != str(ADMIN_ID): return
    service_name = message.text.strip()
    link = generate_active_service_link(service_name)
    
    markup = InlineKeyboardMarkup().add(
        InlineKeyboardButton("🚀 فتح صفحة التفعيل الفورية", url=link)
    )
...
# ==========================================
# 8. الشراء الفعلي وتوليد رابط التفعيل الفعال
# ==========================================
